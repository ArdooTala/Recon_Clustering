...
def _form_reciprocal_dependency_group(graph: nx.DiGraph, subgraph, extend=True):
...
def _find_reciprocal_dependency_groups(graph):
    ...
    reciprocal_dependencies_groups2 = []
    for scc in condensed:
        ...
        reciprocal_dependencies_groups2.append(
            _form_reciprocal_dependency_group(graph, scc_nodes, False)
        )
    ...
    rdgs_nodes2 = [n for rdg in reciprocal_dependencies_groups2 for n in rdg]
    logger.debug(f"FOUND {len(reciprocal_dependencies_groups2)} RECIPROCAL DEPENDENCY GROUPS > # of Nodes/Unique Nodes: {len(rdgs_nodes2)}/{len(set(rdgs_nodes2))}")
    logger.debug("\n")

    assert set(sorted(rdgs_nodes)) == set(sorted(rdgs_nodes2))
